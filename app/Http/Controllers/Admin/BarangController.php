...
class BarangController extends Controller
{
    ...
    public function index()
    {
        ...
        $data["bmharga"] = BarangMasukModel::orderBy('bm_id', 'DESC')->get();
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = BarangModel::leftJoin('tbl_jenisbarang', 'tbl_jenisbarang.jenisbarang_id', '=', 'tbl_barang.jenisbarang_id')->leftJoin('tbl_satuan', 'tbl_satuan.satuan_id', '=', 'tbl_barang.satuan_id')
            ->leftJoin('tbl_merk', 'tbl_merk.merk_id', '=', 'tbl_barang.merk_id')->orderBy('barang_id', 'DESC')->get();
            return DataTables::of($data)
                ...
                //     $replace = Str::replace($barangharga,$jmlhargaupdate,$barangharga);
                //     return $replace;    
    ...
    public function proses_tambahb(Request $request){
        $img= "";
        $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $request->nama)));
        ...
        if ($request->file('foto')==null){
            ...
            $image = $request->file('foto');
            $image->storeAs('public/barang', $image->hasName());
            $img = $image->hashName();
        }
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) use ($request) {    
                    ...
                    $array = array(
                        "barang_kode" => $row->barang_kode,
                        // "totalstok" => trim(preg_replace('/[^A-Za-z0-9-]+/', '_', $totalstok)), 
                        "totalstok" => $totalstok,
                        // "totalstok" => $totalstok,
                        "barang_nama" => trim(preg_replace('/[^A-Za-z0-9-]+/', '_', $row->barang_nama)),
