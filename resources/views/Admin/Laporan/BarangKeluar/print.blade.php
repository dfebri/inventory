...
<body onload="window.print()">
    ...
    {{-- <center>
        ...
        @else
        <img src="{{asset('storage/web/' . $web->web_logo)}}" width="150px" alt="">
        @endif
    </center> --}}

    <center>
        <h2 class="font-medium">Laporan Barang Keluar</h2>
        @if($tglawal == '')
    ...
    <table border="1" id="table1">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$d->bk_kode}}</td>
                <td align="center">{{Carbon::parse($d->bk_tanggal)->translatedFormat('d F Y')}}</td>
                <td>{{$d->barang_nama}}</td>
                <td align="center">{{'Rp '. number_format($d->barang_harga)}}</td>
                <td align="center">{{$d->bk_jumlah}}</td>
                <td align="center">{{$d->bk_tujuan}}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
